...
void getCharFreq(int charFreq[256], uint8_t* fin, int inputFileSize) {
...
void writeCodeToBuffer(uint32_t* fout, int* offset, SymbolCode sc) {
    int numElems = ceil((double) sc.codeLen / 32);
    for(int i = 0; i < numElems; i++) {
        int numBits = 32;
        if(sc.codeLen % 32 != 0 && i == numElems - 1)
    ...
    *bitOffset = 0;
    ...
    for(int i = 0; i < 256; i++) {
        if(codeTable[i].codeLen > 0) {
            ...
            writeCodeToBuffer(fout, bitOffset, codeTable[i]);
        }
    }
}

void encodeFile(uint32_t* fout, int* bitOffset, uint8_t* fin, int inputLengthBytes, SymbolCode codeTable[256]) {
    for(int i = 0; i < inputLengthBytes; i++) {
        writeCodeToBuffer(fout, bitOffset, codeTable[fin[i]]);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if(argc != 3 || (argc > 1 && strcmp(argv[1], "--help") == 0)) {
        fprintf(stderr, "Usage: %s <input text file> <output file name>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    uint8_t* fin = (uint8_t*) mmap(NULL, inputFileSize, PROT_READ, MAP_PRIVATE, in_fd, 0);

    // measure char frequency for ASCII set
    int charFreq[256] = {0};
    getCharFreq(charFreq, fin, inputFileSize);
    ...
    uint32_t* fout = (uint32_t*) mmap(NULL, outputFileSize, PROT_READ | PROT_WRITE, MAP_SHARED, out_fd, 0);
